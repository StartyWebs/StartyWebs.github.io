<div class="container-fluid page-header py-5">
    <div class="container text-center py-5">
        <h1 class="display-2 text-white mb-4 animated slideInDown">Vídeos</h1>
    </div>
</div>
...
<div class="container-fluid project py-5 mb-5">
    <div class="container">
        ...
        <div class="row g-5">
			...
			$Pager = new Pager(BASE . "/videos/", "<<", ">>", 3);
            $Pager->ExePager($Page, 12);
			$Read->ExeRead(DB_VIDEO, "WHERE video_status = 1 AND video_date <= NOW() ORDER BY video_id DESC LIMIT :limit OFFSET :offset", "limit={$Pager->getLimit()}&offset={$Pager->getOffset()}");
            if (!$Read->getResult()):
                $Pager->ReturnPage();
                echo Erro("Ainda Não existem vídeos cadastrados. Favor volte mais tarde :)", E_USER_NOTICE);
            else:
            	$delay = 0.3;
                foreach ($Read->getResult() as $LastPDT):
                    extract($LastPDT);
					
					$embed_url = Check::getVideoEmbedInfo($video_link);
